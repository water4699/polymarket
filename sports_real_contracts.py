...
def get_contract_addresses(market):
    ...
    if condition_id:
        ...
        # 生成CLOb Token IDs（如果有的话）
        clob_tokens = market.get("clobTokenIds", [])
        if clob_tokens:
            contract_info["clob_token_ids"] = clob_tokens

    # 添加已知的Polymarket合约地址
    contract_info["known_contracts"] = REAL_CONTRACTS.copy()
...
def main():
    """主函数"""
    print("🚀 Polymarket 体育市场抓取工具（包含真实合约地址）")
    print("=" * 60)
    ...
    if sports_markets:
        ...
        for i, market in enumerate(sports_markets, 1):
            ...
            # 显示合约地址
            known_contracts = market.get('known_contracts', {})
            if known_contracts:
                print("   合约地址:")
                for name, address in known_contracts.items():
