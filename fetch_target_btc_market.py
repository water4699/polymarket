...
def get_target_market():
    """获取目标比特币市场"""
    market_id = "1119660"  # 目标市场ID
    
    url = f"{GAMMA_BASE}/markets/{market_id}"
    
    print(f"🎯 获取目标市场: {market_id}")
...
def save_market_data(market_data, filename):
    ...
    data_to_save = {
        "metadata": {
            ...
            "fetch_method": "target_market_id_lookup",
...
def fetch_all_transactions(market, polygon_client):
    ...
    try:
        # 1. 获取市场交易数据 (ERC-1155转账)
        if condition_id:
            print("\n🏷️ 获取ERC-1155转账...")
            # 使用现有的方法获取交易
            market_trades = polygon_client.get_market_trades_by_condition_and_token(
                condition_id=condition_id,
                limit=1000  # 获取更多交易
            )
            all_transactions['erc1155_transfers'] = market_trades.get('trades', [])
        ...
        # 2. 获取ERC-20转账 (暂时跳过，因为没有专用方法)
        print("\n💰 获取ERC-20转账...")
        all_transactions['erc20_transfers'] = []
        all_transactions['metadata']['data_sources'].append('erc20_transfers')
        print("   ⚠️ ERC-20转账: 暂无专用方法，跳过")

        # 3. 获取合约调用 (暂时跳过，因为没有专用方法)
        print("\n⚙️ 获取合约调用...")
        all_transactions['contract_calls'] = []
        all_transactions['metadata']['data_sources'].append('contract_calls')
        print("   ⚠️ 合约调用: 暂无专用方法，跳过")
...
def save_transactions_data(transactions_data, filename):
    """保存交易数据到文件"""
    os.makedirs(DATA_DIR, exist_ok=True)
    filepath = os.path.join(DATA_DIR, filename)
...
def main():
    ...
    market_filename = f"target_btc_market_{timestamp}.json"
    save_market_data(market, market_filename)
    
    # 5. 初始化Polygon客户端并获取交易数据
    try:
        print("\n🔧 初始化Polygon客户端...")
        polygon_client = PolygonClient(config.postgres_url)
        ...
        if transactions_data:
            trades_filename = f"target_btc_polygon_trades_{timestamp}.json"
            save_transactions_data(transactions_data, trades_filename)
